<template>
  <div>
    <div id="topInfo" :style="{'opacity': infoOpacity}">
      ...
      <p v-else><span id="topInfo-sorry">Sorry,</span> something is not working. Checkout <br> <router-link class="underline pointer" to="/help">help/supported-devices</router-link> section on how to fix it.</p>
    </div>

    <div id="paintings">
      <div v-show="p._loaded==true" class="painting" v-for="p in paintings" :key=p.id :class="p.width > p.height ? 'breitBild' : ''"  @click="openXRWithPainting(p.id)">
